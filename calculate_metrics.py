...
# existing FID/FD-DINOv2 ref pkl without recomputing the FID entries. The
# top-level image count remains the largest count represented in the file.
    ...
    if merge_existing and os.path.isfile(path):
        try:
            ...
            if isinstance(existing, dict):
                ...
                merged.update(out)
                if 'num_images' in existing and 'num_images' in out:
                    merged['num_images'] = max(existing['num_images'], out['num_images'])
                out = merged
...
def calculate_metrics_from_stats(
    ...
    for metric in metrics:
        ...
        if stat_type == 'moments':
            ...
            # SciPy <1.16 accepted ``disp=False`` and returned ``(sqrt, err)``;
            # newer releases removed that argument and always return the
            # matrix square root directly. Calling without ``disp`` works in
            # both APIs.
